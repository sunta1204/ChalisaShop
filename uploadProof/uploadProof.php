...
<?php 
	...
	for ($i=0 ; $i < count($_FILES['proofConfirm']['name']) ; $i++) { 
		if ($_FILES['proofConfirm']['name'] != "") {
			...
			if (move_uploaded_file($_FILES['proofConfirm']['tmp_name'][$i], "../proofConfirm/" . $nameImg)) {
	...
	//$target2 = "../proofPayment/" . basename($_FILES['proofPayment']['name']);

	$image2 = $order_id . ".jpg";

	move_uploaded_file($_FILES['proofPayment']['tmp_name'], "../proofPayment/" . $image2);
	...
	if($stmt->execute()){
		echo "<script type='text/javascript'> window.location.href = '../addressForm.php';</script>";
	}else{
